...
for i,feature in enumerate(possible_features):
    ...
    main_obj.draw_diagrams(save=False, show=True)
    main_obj.to_blender()
    ...
    vis.add_curve(edited_times, x=og_times, fig=fig, row=1, col=2, name=feature, color=edited_curve.color)
    if i==0: vis.add_curve(gt_times, x=og_times, fig=fig, row=1, col=2, name="Ground truth", color=og_curve.color)
    ...
    vis.add_pairings(x1=x1, y1=y1, x2=x2, y2=y2+offset, pairs=internal.dtw.pairings, fig=fig3)
