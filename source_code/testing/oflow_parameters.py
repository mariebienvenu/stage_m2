...
for level in range(1,101):
    oflow = optical_flow.compute_oflow(frame1, frame2, levels=level)
    bgr = optical_flow.make_oflow_image(oflow)
    cv2.imwrite(data_path+f'/optical_flows_levels/level_{level}.png', bgr)

for winsize in range(15,700, 10):
    oflow = optical_flow.compute_oflow(frame1, frame2, winsize=winsize)
    bgr = optical_flow.make_oflow_image(oflow)
    cv2.imwrite(data_path+f'/optical_flows_window_size/winsize_{winsize}.png', bgr)

for iterations in range(1,32):
    oflow = optical_flow.compute_oflow(frame1, frame2, iterations=iterations)
    bgr = optical_flow.make_oflow_image(oflow)
    cv2.imwrite(data_path+f'/optical_flows_iterations/iterations_{iterations}.png', bgr)

for polynomial in range(3,100, 2):
    oflow = optical_flow.compute_oflow(frame1, frame2, poly_n=polynomial, poly_sigma=1.3+(polynomial-2)*0.25)
    bgr = optical_flow.make_oflow_image(oflow)
